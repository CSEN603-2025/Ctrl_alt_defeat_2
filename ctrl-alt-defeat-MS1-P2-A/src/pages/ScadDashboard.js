...
import ScadAppointments from './ScadAppointments'; // Added import
...
\\textbf{Cycle:} ${evaluation.cycle}
  ...
  const handleSetCycleDates = () => {
    ...
    if (new Date(cycleStartDate) >= new Date(cycleEndDate)) {
      setDownloadMessage('❌ End date must be after start date');
      setTimeout(() => setDownloadMessage(''), 3000);
      return;
    }

    // Here you would typically save these dates to your backend
    setNotificationMessage('Internship cycle dates set successfully');
    setShowNotification(true);
    setShowCycleModal(false);
    
    // Show feedback message
    setDownloadMessage('✔️ Internship cycle dates set successfully');
    setTimeout(() => setDownloadMessage(''), 3000);
  };
  ...
  const renderContent = () => {
    switch (activeSection) {
      ...
      case 'internship-postings':
        return (
          <div className="internship-section animated fadeInUp">
            <div className="cycle-dates-button-container">
              <button 
  ...
  <section className="scad-hero-banner scad-animated scad-fadeSlideUp">
    <h2>
      {activeSection === 'internship-postings' ? 'Internship Postings' : 
